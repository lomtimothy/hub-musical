<x-layouts::app :title="__('Reservar sesión')">
    ...
    <div class="mt-4 space-y-4">
        @if ($maxAdditionalParticipants > 0)
            @foreach (range(0, $maxAdditionalParticipants - 1) as $index)
            ...
            @endforeach
        @else
            <p class="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-300">
                Este estudio no permite agregar músicos adicionales porque su capacidad es de una persona.
            </p>
        @endif
    </div>
</div>
